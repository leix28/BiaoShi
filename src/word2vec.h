...
void DeleteVocab() {
	for (int i = 0, vocabSize1 = vocabSize, vocabSize = 0; i < vocabSize1; i++)
...
void LoadFromTrainFile(char *route) {
    ...
    if (feof(fin)) break;
		...
		vocabList[tmp].f++;
		while (vocabSize > hashSize * hashLimit) DeleteVocab();
	}
...
void InitNegativeTable() {
	...
	for (int i = 0; i < vocabSize; i++) tot += pow(vocabList[i].f, power);
	real j = pow(vocabList[0].f,power) / tot;
	for (int i = 0, step = 0; i < NegativeTableSize; i++) {
...
void InitTree() {
	...
	long long *father = (long long *)calloc(vocabSize * 2 + 1, sizeof(long long));
  	...
  	for (int i = 0; i < vocabSize; i++) count[i] = vocabList[i].f;
  	for (int i = vocabSize; i < (vocabSize << 1); i++) count[i] = MAXINITIAL;

	for (int i = 0, lef = vocabSize - 1, rig = vocabSize; i < vocabSize - 1; i++) {
		int minimal[2];
		for (int j = 0; j < 2; j++)
	...
	for (int i = 0; i < vocabSize; i++) {
		int k = 0;
    	for (int j = i; j != (vocabSize << 1)-2; j = father[j], k++) {
    		code[k] = binary[j];
    		point[k] = j;
    	}
...
void OutEmbeddings() {
	...
	for (int i = 0; i < vocabSize; i++) {
		...
		if (binary) 
			for (int j = 0; j < dimension; j++) fwrite(&syn0[i * dimension + j], sizeof(real), 1, fout);
		else
			for (int j = 0; j < dimension; j++) fprintf(fout, "%lf ", syn0[i * dimension + j]);
		fprintf(fout, "\n");
	}
	fclose(fout);
}
  ...
  while (1) {
    ...
    if (sentence_length == 0) {
      while (1) {
        ...
        if (sample > 0) {
          real ran = (sqrt(vocabList[word].f / (sample * wordFrequency)) + 1) * (sample * wordFrequency) / vocabList[word].f;
